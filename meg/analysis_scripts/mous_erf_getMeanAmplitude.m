...
% time samples for interval of interest

    % target word: 0.2 - 0.5s
    intStart = 226;
    intEnd   = 301;

    % tar + 1 (not timelocked): 1.1 - 1.4s
    % intStart = 481;
    % intEnd   = 571; 

    % tar + 1 (not timelocked): 1.75 - 2.05s
    % intStart = 676;
    % intEnd   = 766; 

% Values for table in SPSS:  subjects * sensor groups (8 for each condition)
MeanAmpTable = zeros(numel(subjlist),sensorgroups);  

%% create table

for subjCount = 1:numel(subjlist) 
    for roiCount = 1:numel(roi)
    % select the channels defined by the roi vector
    % Sentences
    [tmp] = ft_selectdata_old(erfSenTar{subjCount},'param','avg','avgoverchan','yes','channel', roi(roiCount).channel);   
    erfSenTar_Clust{roiCount} = tmp;
    erfSenTar_Clust{roiCount}.clustername = roi(roiCount).label;
    MeanAmpTable(subjCount,roiCount) = mean(erfSenTar_Clust{roiCount}.avg(:,intStart:intEnd));
   
    % Sequences
    [tmp] = ft_selectdata_old(erfSeqTar{subjCount},'param','avg','avgoverchan','yes','channel', roi(roiCount).channel);   
    erfSeqTar_Clust{roiCount} = tmp;
    erfSeqTar_Clust{roiCount}.clustername = roi(roiCount).label;
    MeanAmpTable(subjCount,roiCount+8) = mean(erfSeqTar_Clust{roiCount}.avg(:,intStart:intEnd));
    end
end
cd /home/language/annhul/MOUS/Processed/groupERF
xlswrite(strcat('ERF_MeanAmpVal_', num2str(numPtp),'ptp_Lay2_','TarWord250-500ms_',date), MeanAmpTable);
