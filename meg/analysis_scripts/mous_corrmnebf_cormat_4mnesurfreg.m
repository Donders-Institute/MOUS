function mous_corrmnebf_cormat_4mnesurfreg(subjectname,param)

% param.range       = 'low'; % all the low frequencies: [2.5 5 7.5 10 12]
% param.foi         = 5;        
...
doavgxtrials                = true;  % average across trials, independently, for osc and erfs before computing correlation matrix
                                     % try this to see if it will improve
                                     % SnR, but it does mean only 1 value per subject
if doselfreq
    %% OSCILLATIONS select frequency 
    if strcmp(param.suff,'_high')
       rootdir = savdir;   % bfica_freq_high is in NL dir.d
    else
       mous_db_getdata(subjectname,['meg_bfica_freq_',param.range],rootdir);  % bfica_freq_low/medium is in JM dir.
    end 
...
if doselerf
    ...
    % new filename for ERFs
    mous_db_getdata(subjectname, 'meg_processed_erf_allwords_01-10'); % AH dir
    ...
    % because of matlab rounding/nearest function/precision issues produces trials with either 30 or 31 samples. 
    % pick the one that gives more trials but not both because trial length
    % must be the same across trials
    tmp = floor(erftime*300); % make integer instead of with decimals i.e. XX.000)
    tmp2 = tmp+1;
        
    nsmp = cellfun('size',data.trial,2); % numsamples for each trial
    numeltmp = numel(find(nsmp == tmp));
    if isempty(numeltmp)  % incase no trial has this # of samples
        numeltmp = 1;
    end 
    numeltmp2 = numel(find(nsmp == tmp2));
    if isempty(numeltmp2) % incase no trials has this # of samples
        numeltmp2 = 1;
    end 
    
    if numeltmp > numeltmp2   
        fulltrial = find(ismember(nsmp,tmp));    
    else 
        fulltrial = find(ismember(nsmp,tmp2));
    end 
...
if domnesingle_beam_reg_cor
    ...
    for mm = 1:2  % first loop = sentences; second loop = sequences
        ...
        if mm == 1
            mous_db_putdata(subjectname, ['meg_corrmnebf_bfsourcesingletrial8mm_fisher_bf',param.savebf,'mne',param.savemne,'_',param.suff,'Hz_sen'],'source','trialinfo',savdir);
        else
            mous_db_putdata(subjectname, ['meg_corrmnebf_bfsourcesingletrial8mm_fisher_bf',param.savebf,'mne',param.savemne,'_',param.suff,'Hz_seq'],'source','trialinfo',savdir);
        end  
        ...
        % get volume conductor that is surface registered (as opposed to
        % previously having to use 3D volume registered model by interpolating cortical sheet to 3D space)
        % in this way the vertices are matched across subjects
        
        %%% FIXME:  new files computer for volume conductor - currenty density  vs. current density weighted
        if mm == 1   
            mous_db_getdata(subjectname,'meg_mne_allwords_01-10-sent',mnedir); % contrary to name, the filter is calculated from using all words (not just target)
        elseif mm == 2;
            mous_db_getdata(subjectname,'meg_mne_allwords_01-10-seq', mnedir);
        end 
        ...
        % compute leave-one-out estimates for the oscillations (leave-one-out estimates for mne are done within do mnesingle)
        voxMsum = sum(voxM,2);
        voxM    = (voxMsum*ones(1,size(voxM,2)) - voxM)./(size(voxM,2)-1);
        
        % regress out word order
        [voxM, vertM] = mous_corrmnebf_regression(trialinfo,voxM,vertM); % 4th argument = number of bins
        ...
        if mm == 1
            mous_db_putdata(subjectname,['meg_corrmnebf_corVoxvert8mm_mnesurfreg_jack_indregword_fisher_avgsingletrial_bf',param.savebf,'mne',param.savemne,'_',param.suff,'Hz_sen'],'cor',savdir); 
        elseif mm == 2
            mous_db_putdata(subjectname,['meg_corrmnebf_corVoxvert8mm_mnesurfreg_jack_indregword_fisher_avgsingletrial_bf',param.savebf,'mne',param.savemne,'_',param.suff,'Hz_seq'],'cor',savdir); 
        end 
    end 
end
