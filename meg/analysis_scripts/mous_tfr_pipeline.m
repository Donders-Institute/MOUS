...
% cfgf1.taper       = 'hanning'; remove this so default = 'dpss' which is multitapering;
cfgf1.foi         = 2.5:2.5:30;                      % analysis 2 to 30 Hz in steps of 2.5 Hz (match t_ftimwin)
cfgf1.t_ftimwin   = ones(1,numel(cfgf1.foi)).*0.4;   % try 400ms to better analyze beta band activity, 12.6.2012. = 2.5cycle per second i.e. 2.5Hz
cfgf1.tapsmofrq   = ones(1,numel(cfgf1.foi)).*5;     % width of frequency smoothing should be an integer multiple of number of cycles in 1s 
cfgf1.toi         = -0.25:0.05:2.75;                 % time window of interest: in steps of 50ms
cfgf1.pad         = 4.0;
...
mous_db_putdata(subjectname, 'meg_processed_{tfr_targetword_05-3ds}',    TFRHann_Diff, TFRMult_Diff, TFRHann_SenTar, TFRHann_SeqTar, TFRMult_SenTar, TFRMult_SeqTar);
mous_db_putdata(subjectname, 'meg_processed_{tfr_targetword_05-3ds-pg}', TFRHann_Diff_PG, TFRMult_Diff_PG, TFRHann_SenTar_PG, TFRHann_SeqTar_PG, TFRMult_SenTar_PG, TFRMult_SeqTar_PG);
