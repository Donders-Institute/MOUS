function [mi] = mous_neuralspeech_mi_crossfreq(subjectname)
...
cfgf1 = [];
cfgf1.bpfilter = 'yes';
cfgf1.bpfilttype = 'firws';

cfgf2 = [];
cfgf2.bpfilter = 'yes';
cfgf2.bpfilttype = 'firws';
cfgf2.bpfreq = [30 50];
...
cfg = [];
cfg.method = 'mi';
cfg.mi.lags = (-0.5:0.1:0.5);

freqs = [linspace(0.5,16,20);linspace(1,20,20);linspace(1.5,24,20)];
data  = ft_preprocessing(cfgh2, ft_preprocessing(cfgf2, data));

for k = 1:size(freqs,2)
  cfgf1.bpfreq = freqs([1 3],k)';

  tmpspeech = ft_preprocessing(cfgh1, ft_preprocessing(cfgf1, speech));
  
  tmp         = ft_appenddata([], data, tmpspeech);
  cfg.refindx = match_str(tmp.label, 'audio_avg');

  mi(k)       = ft_connectivityanalysis(cfg,tmp);
end
