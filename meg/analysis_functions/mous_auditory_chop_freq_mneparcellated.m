function freq = mous_auditory_chop_freq_mneparcellated(subjectname)
...
for k = 1:numel(f)
  ...
  tcl(:,3) = 0; % trialfun_auditory_sentence defines time point 0 to be the first word onset
...
mous_db_getdata(subjectname,'meg_mne_target_02-10_sent');
W = zeros(3*size(source.pos,1),size(source.avg.filter{source.inside(1)},2));
for k = source.inside(:)'
  indx = (k-1)*3+(1:3);
  W(indx,:) = source.avg.filter{k};
end

load atlas_conte69_8196reg_LR_brodmann_subparc;
p = zeros(size(atlas.pos,1),max(atlas.parcellation));
for k = 1:size(p,2)
  p(atlas.parcellation==k,k) = 1;
end
P = zeros(size(p')*3);
P(1:3:end,1:3:end) = p';
P(2:3:end,2:3:end) = p';
...
params.W = P*W;
clear P W;
label = cell(numel(atlas.parcellationlabel)*3,1);
for k = 1:numel(atlas.parcellationlabel)
  label{(k-1)*3+1} = [atlas.parcellationlabel{k},'_x'];
  label{(k-1)*3+2} = [atlas.parcellationlabel{k},'_y'];
  label{(k-1)*3+3} = [atlas.parcellationlabel{k},'_z'];
end
...
mous_db_putdata(subjectname, 'meg_mne_auditory_freqitc_parcellated', 'freq');
