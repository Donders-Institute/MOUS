...
for j = 1:length(AllFiles)
    ...
    try
        ...
        for k = 1:size(resp,1)
            ...
            SubnrInd = regexpi(resp{k},'V\d\d\d\d');
        ...
        %% Save data
        if ~exist('SingleTrials_Vis_meg','var')
            SingleTrials_Vis_meg = QuestionNr;
        else
            SingleTrials_Vis_meg = [SingleTrials_Vis_meg;QuestionNr];
        end
...
save('SingleTrials_Vis_meg','SingleTrials_Vis_meg')