function mous_bfica_parcellate_statistics_SevlWevl(freq,chansel, bsltype)
...
% Sentence Early-late words vs.  Word list Early-late words
...
% sourcedata
savename = 'sourcedataWordpositionSvWL';
sourcedat1 = '_bfica_sourcedata_earlySEN_matched_';
sourcedat2 = '_bfica_sourcedata_lateSEN_matched_';

sourcedat3 = '_bfica_sourcedata_earlyWL_matched_';
sourcedat4 = '_bfica_sourcedata_lateWL_matched_';
...
% easier way to produce a repmat of the baseline time window
% s1{k,1}.ptseries = s1{k,1}.ptseries - s1{k,1}.ptseries(:,ones(1,13));
...
elseif strcmp(bsltype,'comabs')  
  for k = 1:Nsubj
    ...
    % s1
    tmp = s1{k}.ptseries - bsl*ones(1,numel(s1{k}.time));  % remove baseline
...
% create conditions for statistics
for k = 1:numel(subj)
  senEvL{k}           = s1{k};
  senEvL{k}.ptseries  = s1{k}.ptseries - s2{k}.ptseries;

  wlEvL{k}            = s1{k};
  wlEvL{k}.ptseries   = s3{k}.ptseries - s4{k}.ptseries;
end
...
statSvWL      = ft_timelockstatistics(cfg, senEvL{:},wlEvL{:});

% posthoc
statsenEvL    = ft_timelockstatistics(cfg,s1{:},s2{:}); % earlySEN - lateSEN
statwlEvL     = ft_timelockstatistics(cfg,s3{:},s4{:}); % earlyWL  - lateWL 
...
%% save
if isfield(cfg,'channel') && numel(cfg.channel) == 32
  save([savedir,savename,'_',frqall,toiall,'_','LIFGLTEMP_',bsltype,'_',num2str(Nsubj),'subj.mat'],...
       'statSvWL','statsenEvL','statwlEvL','Nsubj',...
       'avgs1','avgs2','avgs3','avgs4','sems1','sems2','sems3','sems4','-v7.3');  
elseif isfield(cfg,'channel') && numel(cfg.channel) == 64
   save([savedir,savename,'_',frqall,toiall,'_','LR_IFGTEMP_',bsltype,'_',num2str(Nsubj),'subj.mat'],...
       'statSvWL','statsenEvL','statwlEvL','Nsubj',...
       'avgs1','avgs2','avgs3','avgs4','sems1','sems2','sems3','sems4','-v7.3');  
else
  save([savedir,savename,'_',frqall,toiall,'_',bsltype,'_',num2str(Nsubj),'subj.mat'],...
       'statSvWL','statsenEvL','statwlEvL','Nsubj',...
       'avgs1','avgs2','avgs3','avgs4','sems1','sems2','sems3','sems4','-v7.3');  
end
