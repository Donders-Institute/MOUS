function tlck = mous_auditory_chop_earlylate(subjectname)
...
wordindx = cell(1,numel(data.trial));
duration = cell(1,numel(data.trial));
for k = 1:numel(data.trial)
  ...
  if isfield(stimuli(idx).words, 'onset')
    ...
    wordindx{k} = 1:stimuli(idx).numwords;
    ...
    for m = 1:numel(onset)
      ...
      if m<numel(onset)
        pst{k}(m) = nearest(data.time{k},onset(m+1))-indx{k}(m) + 120; % add 0.1 second beyond the onset of the next word
      else
        pst{k}(m) = numel(data.time{k})-indx{k}(m);
      end
    end
    end
    dur = diff(stimuli(mod(idx,500)).timinginfo(:,2));
    duration{k} = dur(stimuli(mod(idx,500)).wordorder);
  end
end
...
for m = 1:6
  for k = 1:numel(data.trial)
    ...
    tmpdur{k}  = [];
    if ~isempty(indx{k})
      if m==1 && data.trialinfo(k,5)<500,
        sel = 1;
      elseif m==2 && data.trialinfo(k,5)<500,
        sel = 2:4;
      elseif m==3 && data.trialinfo(k,5)<500,
        sel = sort(wordindx{k}(end)-(1:3));
      elseif m==4 && data.trialinfo(k,5)>500,
        sel = 1;
      elseif m==5 && data.trialinfo(k,5)>500,
        sel = 2:4;
      elseif m==6 && data.trialinfo(k,5)>500,
        sel = sort(wordindx{k}(end)-(1:3));
      else
        % this sentence does not contribute
        sel = [];
      end
      tmppre{k} = pre{k}(sel);
      tmppst{k} = pst{k}(sel);
      tmpindx{k} = indx{k}(sel);
      tmpdur{k} = duration{k}(sel);
      if m==1 || m==4
        tmpdur{k} = tmpdur{k}*4; % artificially increase the length, we are interested in baseline activity to begin with
      end
    end
  
  end
  tlck{m} = compute_singleword(data, mask, tmpindx, tmppre, tmppst, tmpdur);
  dof(m,:,:) = tlck{m}.dof;
end
tlck = ft_appendtimelock([], tlck{:});
tlck.grad = data.grad;
tlck.dof  = dof;

function tlck = compute_singleword(data, mask, indx, pre, pst, duration)
