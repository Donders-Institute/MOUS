function tlck = mous_auditory_chop_parametric(subjectname)
...
pre = cell(numel(data.trial));
pst = cell(numel(data.trial));
...
for m = 1:10
  for k = 1:numel(data.trial)
    ...
    tmppst  = pst;
    if ~isempty(indx{k})
      if numel(wordindx{k})>=m
        sel = wordindx{k}(m);
      else
        sel = [];
      end
      tmppre{k} = tmppre{k}(sel);
      tmppst{k} = tmppst{k}(sel);
      tmpindx{k} = tmpindx{k}(sel);
    end
  
  end
  tlck{m} = compute_singleword(data, mask, tmpindx, tmppre, tmppst);
  dof(m,:,:) = tlck{m}.dof;
end
...
function tlck = compute_singleword(data, mask, indx, pre, pst)
