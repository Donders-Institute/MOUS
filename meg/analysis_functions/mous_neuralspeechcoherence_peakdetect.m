function [varargout] = mous_neuralspeechcoherence_peakdetect(subjectname)

if ischar(subjectname)
  mous_db_getdata(subjectname, 'meg_coh_sensor');

  [varargout{1},varargout{2}] = mous_neuralspeechcoherence_peakdetect(sentcoh);
  [varargout{3},varargout{4}] = mous_neuralspeechcoherence_peakdetect(wlcoh);
  return;
...
X = data.cohspctrm;
for k = 1:size(X,1)
  X(k,:) = mysmooth(X(k,:), 0.05, 'rloess');
end

% determine data-dependent threshold
thr  = median(X(:))+iqr(X(:))./2;
cnt  = zeros(size(X,1),numel(data.freq));
wcnt = cnt;
for k = 1:size(X,1)
  [p,v] = peakdetect3(X(k,:), thr);
  cnt(k,p)  = 1;
  wcnt(k,p) = v;
end

wcnt         = ft_preproc_smooth(wcnt, gausswin(11,5));
[pbin, pval] = peakdetect3(trimmean(wcnt), median(trimmean(wcnt))+iqr(trimmean(wcnt))./2);

data.cohspctrm_smooth = X;
data.wcnt             = trimmean(wcnt);

varargout{1} = data.freq(pbin);
varargout{2} = data;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
function [c,ww] = mysmooth(varargin)
