function mous_bfica_parcellate_statistics_wrdpstncomplexity_sent(freq,chansel,bsltype)
...
savedir  = '/project/3011020.09/nielam/groupresults/bfica/visual/complexity/WdpstnByComplexity_cdtncommabsolutebsl_sen/';

% Input file specfications:
% sourcedata
savename = 'sourcedataWordpositionBycomplexity';
sourcedat1 = '_bfica_sourcedata_sent_earlyRC_matched_';
sourcedat2 = '_bfica_sourcedata_sent_lateRC_matched_';

sourcedat3 = '_bfica_sourcedata_sent_earlyMX_matched_';
sourcedat4 = '_bfica_sourcedata_sent_lateMX_matched_';
...
%% Collapse levels between factors to test
  for k = 1:numel(subj)
    emxlrc{k}         = s1{k};
    emxlrc{k}.ptseries = s3{k}.ptseries + s2{k}.ptseries;

    lmxerc{k}         = s1{k};
    lmxerc{k}.ptseries = s4{k}.ptseries + s1{k}.ptseries;

    early{k}          = s1{k};
    early{k}.ptseries  = s1{k}.ptseries + s3{k}.ptseries;

    late{k}           = s1{k};
    late{k}.ptseries   = s2{k}.ptseries + s4{k}.ptseries;

    rc{k}             = s1{k};
    rc{k}.ptseries     = s1{k}.ptseries + s2{k}.ptseries;

    mx{k}             = s1{k};
    mx{k}.ptseries     = s3{k}.ptseries + s4{k}.ptseries;
  end
  ...
  stattime = ft_timelockstatistics(cfg, early{:}, late{:});

  cfg.design    = [ones(1,Nsubj) ones(1,Nsubj)*2;1:Nsubj 1:Nsubj];
  statcomplex = ft_timelockstatistics(cfg, rc{:}, mx{:});

  cfg.design    = [ones(1,Nsubj) ones(1,Nsubj)*2;1:Nsubj 1:Nsubj];
  stattimecomp = ft_timelockstatistics(cfg, emxlrc{:}, lmxerc{:});
...
if isfield(cfg,'channel') && numel(cfg.channel == 64)
  save([savedir,savename,'_',frqall,toiall,'_commabsbsl_','LIFGLTEMP_',num2str(Nsubj),'subj.mat'],...
     'stattime','statcomplex','stattimecomp','Nsubj',...
     'avgs1','avgs2','avgs3','avgs4','sems1','sems2','sems3','sems4','-v7.3');    
else
  save([savedir,savename,'_',frqall,toiall,'_commabsbsl_',num2str(Nsubj),'subj.mat'],...
     'stattime','statcomplex','stattimecomp','Nsubj',...
     'avgs1','avgs2','avgs3','avgs4','sems1','sems2','sems3','sems4','-v7.3');    
end
