function  [corrS, corrM, stat] = rsa_permmodality3(voxelstart,voxelend,latewindow,windowsize)
    ...
    for v = 1:j
        ...
        if any(tmpp(:))
            for i = 1:Nperm
                ...
                %divide rsa matrix into quads and combine one
                %within-modality quad with one supramodal quad

                quads=reshape(tmpp(:,:,:),[Nsubj 2 Nsubj 2 3]);
                % permute supramodal and withinmodal quads (rowwise) for
                % each time window
                withinquadlow = [];
                acrossquadlow = [];
                for tim = 2:3
                ...
                %convert back
                quads=reshape(quads,Nsubj^2,4,3);
