% This script extracts several types of information from the relative
% clause sentences (1:204) and adds them the mous_stimuli.mat file.

% subjMC      = word position of subject in main clause
% verb2subjMC = verb referring to subject in main clause 
% sentencetype = in binary, notes which of the 4 characteristics are
% present.
% 1. adverbial time phrase
% 2 = Right Branching  
% 3 = Object Relative 
% 4 = Non-embedded relative clause

%% N.B. 202 sentences, but sentID is 1:204
%       107 and 164 are missing.
clear all;

% read in file contain coded categories
% 16: subject of main verb
% 17: verb belonging to 16
xlsfile = '/home/language/nielam/MOUS_Stimuli/mous_category_SubjMC_verbRC_NLam.xlsx';
[data_num,data_txt,data_raw] = xlsread(xlsfile,'','','basic'); 

% Replicate sentenceID (column 1) on the even-number rows
nums = data_num(1:2:end,1);
data_num(2:2:end,1) = nums';
data_num(1:2:end,:) = [];

% get locations of 16's and 17's in each sentence
for k = 1:size(data_num,1)
  currsent = data_num(k,2:end);
  i1(k,1) = data_num(k,1);
  i1(k,2) = find(currsent == 16);
  i1(k,3) = find(currsent == 17);
  i1(k,4) = i1(k,3) - i1(k,2);
end

% distribution of distances between subj and verb
hist(i1(:,4),1:10); title('distribution of distances between subj and verb')
figure;hist(i1(:,2),1:15); title('SubjMC distribution')
figure;hist(i1(:,3),1:15); title('verb to SubjMC distribution')

% load in the stimuli struct-array
load mous_stimuli;

%% enter 'subjMC' and 'verb2subjMC' as fields
for k = 1:size(data_num,1)
  sID = data_num(k,1);
  stimuli(sID).subjMC = i1(k,2);
  stimuli(sID).verb2subjMC = i1(k,3);  
end

%% enter sentence type | columns in .xlsfiles | category markers
% 1 = Adverbial phrase |            1         | 20
% 2 = Right Branching  |            2         | 21
% 3 = Object Relative  |            3         | 22
% 4 = Non-embedded relative clause |4         | 23

xlsfile = '/home/language/nielam/MOUS_Stimuli/mous_category_SubjMC_verbRC_NLam.xlsx';
[data_num,data_txt,data_raw] = xlsread(xlsfile,'sentenceType','','basic'); % sentenceType is 2nd sheet in xlsfile

% remove unnecessary rows
data_num(2:2:end,:) = [];
data_num(end,:)     = []; 

% convert category markers into binary code
sentbinarycode = data_num;       %  replicate matrix
idxnum = find(~isnan(data_num)); %  idx of categores
sentbinarycode(idxnum) = true;    
idxnan = setdiff(1:size(data_num,1)*size(data_num,2),idx);  % idx of nans
sentbinarycode(idxnan) = false;
sentbinarycode(:,1) = data_num(:,1);  % return stimuliID

for k = 1:size(sentbinarycode,1)
  sID = sentbinarycode(k,1);
  stimuli(sID).sentencetype = sentbinarycode(k,2:end);  
end
