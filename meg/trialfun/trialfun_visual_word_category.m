...
% respective categories (coded by hand)
    ...
    %% Read in logfile
    [newtxt] = read_logfile_visual;
    
    % log lines from logfile that contain a word that is actual stimuli
    ...
    expwords = cell(240,1);  % from the experimental logfile (expwords) concatenate words
    cnt = 0;
    ...
    [catnum,~,catdata] = xlsread('home/language/nielam/MOUS_AnalysisNotes/GrammDist/Mous_categories.xls','','','basic'); 
    ...
    % idx4expword is the index of the sent/seq found in catwords, not the (audiofile) name of
    ...
    for qq = 1:size(expwords,1)
        ...
        % if no match i.e. stimuli in logfile is wrong due to typo, don't add it to trialfun
        % so far this has been for audfile ID 58, where last word "lachte" is spelled as "achte" 
        if isempty(idxsent)    %log the trial(sent/seq) without a match
            if isempty(removeidx)
                removeidx = qq;
            else 
                removeidx = [removeidx; qq];
            end
        else 
...
fname       = mous_db_getfilename(subjectname, 'meg_ds_task');
prestim     = 0.5; 
poststim    = 3.0;
wordType    = 'all';
trialfun    = 'visual_word';
[trl]       = mous_defineTrial(fname{1}, prestim, poststim, wordType, trialfun); 

% remove trial (sent/seq) from experiment(logfile) that doesn't have a
% match with excel file. Otherwise, when concatenating between
% trialfun_visual_word and word categories, it will not match up.

if ~isempty(removeidx)
    for nn = 1:numel(removeidx)
        ridx = find(trl(:,4) == removeidx(nn));
        trl(ridx,:) = [];
    end 
end 
...
% length of each sentence/sequence based on trl matrix (prior to artifact detection)
triallen = [];
...
catvec = [];
for mm = 1:size(triallen,1)   % number of elements in triallen, not idx4expwords, because some ptps don't have all 240trials
