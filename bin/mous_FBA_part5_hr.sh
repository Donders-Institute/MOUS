...
subject=$1
output_folder=/project/3011020.09/processed/"$subject"/mri_dti/mrtrix_hr
group_folder_dwi=/project/3011020.09/processed/mrtrix_group_hr

# Register the subject specific FOD image to the FOD template
mrregister $output_folder/wmfod.mif -mask1 $output_folder/data_mask_hr.nii.gz $group_folder_dwi/wmfod_template.mif -nl_warp $output_folder/subject2template_warp.mif $output_folder/template2subject_warp.mif -force

# Warp the mask into template space:
mrtransform $output_folder/data_mask_hr.nii.gz -warp $output_folder/subject2template_warp.mif -interp nearest -datatype bit $output_folder/dwi_mask_in_template_space.mif -force
